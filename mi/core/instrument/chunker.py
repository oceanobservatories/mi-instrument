...
class StringChunker(object):
    ...
    def add_chunk(self, raw_data, timestamp):
        ...
        """
        log.info('add_chunk: %r %r', raw_data, timestamp)
    ...
    def _prune_overlaps(results):
        ...
        log.info('Found overlapping matches from sieve function: %r %r', remove_indices, len(results))
    ...
    def _find_timestamp(self, index):
        ...
        """
        log.info('find_timestamp: %r', index)
        for start, stop, timestamp in self.timestamps:
            log.info('find_timestamp LOOP: %r %r %r', start, stop, timestamp)
            if start <= index < stop:
                return timestamp
    ...
    def _rebase_times(self, index):
        ...
        """
        log.info('_rebase_times: %r', index)
        new_times = []
        for start, stop, timestamp in self.timestamps:
            if stop <= index:
                continue
    ...
    def _make_chunks(self):
        ...
        for start, end in results:
            ...
            log.info('found chunk: %r %r', timestamp, chunk)
