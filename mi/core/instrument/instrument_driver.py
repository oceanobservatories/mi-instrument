...
class NoCommandInstrumentDriver(SingleConnectionInstrumentDriver):
    __metaclass__ = META_LOGGER

    def __init__(self, event_callback, refdes=None):
        """
        Creates a command-free instrument driver (receive only).
        :param event_callback: Callback to the driver process to send asynchronous events
        :param refdes: reference designator for the instrument
        """
        SingleConnectionInstrumentDriver.__init__(self, event_callback)

        handlers = {
            DriverState.UNCONFIGURED: [
                (DriverEvent.ENTER, self._handler_unconfigured_enter),
                (DriverEvent.EXIT, self._handler_noop),
                (DriverEvent.INITIALIZE, self._handler_none_state),
                (DriverEvent.CONFIGURE, self._handler_noop),
            ],
            DriverConnectionState.DISCONNECTED: [
                (DriverEvent.ENTER, self._handler_disconnected_enter),
                (DriverEvent.EXIT, self._handler_noop),
            ...
            DriverConnectionState.INST_DISCONNECTED: [
                ...
                (DriverEvent.CONNECT, self._handler_inst_disconnected_connect),
                (DriverEvent.CONNECTION_LOST, self._handler_connected_connection_lost),
                (DriverEvent.PA_CONNECTION_LOST, self._handler_none_state),
            ],
            DriverConnectionState.CONNECTED: [
                (DriverEvent.ENTER, self._handler_connected_enter),
                (DriverEvent.EXIT, self._handler_noop),
    ...
    def refuse_commands(self):
        """
        cowardly refuse to service any command related functions
        """
        raise InstrumentParameterException('Attempt to command a read-only interface denied')
    ...
    def get_direct_config(self):
        """
        direct access configuration is not available for this read-only interface
        """
        config = {}
        return [config]
    ...
    def _handler_none_state(self, *args, **kwargs):
        return None, None

    def _handler_disconnected_connect(self, *args, **kwargs):
        result = None

        # ignore instrument parameter settings by removing args
        return SingleConnectionInstrumentDriver._handler_disconnected_connect()
