...
def main():
    qpid_url = os.environ.get('QPID_URL')
    qpid_queue = os.environ.get('QPID_QUEUE')
    rabbit_url = os.environ.get('RABBIT_URL')
    rabbit_queue = os.environ.get('RABBIT_QUEUE')
    if all((qpid_url, qpid_queue, rabbit_url, rabbit_queue)):
        qpid = QpidProducer(qpid_url, qpid_queue)
        rabbit = RabbitConsumer(rabbit_url, rabbit_queue, qpid)
        rabbit.run()
    else:
        log.error('Missing mandatory ENV variables (QPID_URL, QPID_QUEUE, RABBIT_URL, RABBIT_QUEUE')
