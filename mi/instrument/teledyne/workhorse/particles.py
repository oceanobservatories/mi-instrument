...
class AdcpSystemConfigurationDataParticle(DataParticle):
    ...
    def regex():
        groups = [
            r'Instrument S/N: +(\d+)',
            r'Frequency: +(\d+) HZ',
            r'Configuration: +([a-zA-Z0-9, ]+)',
            r'Match Layer: +(\d+)',
            r'Beam Angle:  ([0-9.]+) DEGREES',
            r'Beam Pattern:  ([a-zA-Z]+)',
            r'Orientation:  ([a-zA-Z]+)',
            r'Sensor\(s\):  ([a-zA-Z0-9 ]+)',
            r'(Pressure Sens Coefficients:)?',
            r'(c3 = ([\+\-0-9.E]+))?',
            r'(c2 = ([\+\-0-9.E]+))?',
            r'(c1 = ([\+\-0-9.E]+))?',
            r'(Offset = ([\+\-0-9.E]+))?',
            r'Temp Sens Offset: +([\+\-0-9.]+) degrees C',
            r'CPU Firmware:  ([0-9.\[\] ]+)',
            r'Boot Code Ver:  Required: +([0-9.]+) +Actual: +([0-9.]+)',
            r'DEMOD #1 Ver: +([a-zA-Z0-9]+), Type: +([a-zA-Z0-9]+)',
            r'DEMOD #2 Ver: +([a-zA-Z0-9]+), Type: +([a-zA-Z0-9]+)',
            r'PWRTIMG  Ver: +([a-zA-Z0-9]+), Type: +([a-zA-Z0-9]+)',
    ...
    def _build_parsed_values(self):
        log.info('build_parsed_values: regex: %s', self.regex())
        log.info('build_parsed_values: raw: %s', self.raw_data)
        results = []
        ...
        results.append(self._encode_value(key.PRESSURE_COEFF_c3, match.group(11), float_or_zero))
        results.append(self._encode_value(key.PRESSURE_COEFF_c2, match.group(13), float_or_zero))
        results.append(self._encode_value(key.PRESSURE_COEFF_c1, match.group(15), float_or_zero))
        results.append(self._encode_value(key.PRESSURE_COEFF_OFFSET, match.group(17), float_or_zero))
