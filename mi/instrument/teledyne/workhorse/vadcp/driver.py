...
from mi.instrument.teledyne.workhorse.driver import WorkhorseParameter, parameter_defaults, WorkhorsePrompt, NEWLINE, \
    ...
    ADCP_ANCILLARY_SYSTEM_DATA_REGEX_MATCHER, ADCP_TRANSMIT_PATH_REGEX_MATCHER, WorkhorseEngineeringParameter, TIMEOUT, \
    WorkhorseScheduledJob
...
class Protocol(WorkhorseProtocol):
    ...
    def _start_logging(self, timeout=TIMEOUT, connection=None):
        ...
        """
        try:
            SL = WorkhorseInstrumentCmds.START_LOGGING
            self._do_cmd_resp(SL, timeout=timeout, connection=SlaveProtocol.FIFTHBEAM)
            self._do_cmd_resp(SL, timeout=timeout, connection=SlaveProtocol.FOURBEAM)
    ...
    def _stop_logging(self):
        for connection in self.connections:
            self._send_break(connection=connection)
    ...
    def _discover(self, connection=None):
        ...
        auto = (WorkhorseProtocolState.COMMAND, ResourceAgentState.STREAMING)
        for connection in self.connections:
            try:
