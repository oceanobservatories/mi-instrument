...
class KMLProtocol(CommandResponseInstrumentProtocol):
    ...
    def _update_metadata_params(self, *args, **kwargs):
        ...
        """
        log.debug("in _update_metadata_params")
        ...
        try:
            ...
            for attr in sorted(cmds):
                ...
                if attr in [ KMLParameter.PAN_POSITION[ParameterIndex.KEY],
    ...
    def _instrument_config_dirty(self):
        ...
        for param in startup_params:
            ...
            if param in [KMLParameter.CAMERA_MODE[ParameterIndex.KEY],
                         # TODO: CAMERA_GAIN needs to be in this list
                         # waiting to sort out instrument issues regarding this parameter
                         #KMLParameter.CAMERA_GAIN[ParameterIndex.KEY],
    ...
    def _parse_get_disk_usage_response(self, response, prompt):
        ...
        response_stripped = '%r' % response.strip()
        #check the size of the response
        if len(response_stripped) != 12:
            raise InstrumentParameterException('Invalid Response received for GC command: Unexpected response size '
                                               + self.get_param + '  ' + response_stripped + ' ' + self.get_cmd)
        if response_stripped[0] != '<' or response_stripped[len(response_stripped) - 1] != '>':
            raise InstrumentParameterException('Invalid Response received for GC command: Instrument response is'
                                               ' incorrectly formatted' + self.get_param + '  ' + response_stripped
                                               + ' ' + self.get_cmd)

        if response_stripped[3] == self.NAK:
            raise InstrumentProtocolException(
                'Protocol._parse_set_response : Set disk usage command not recognized')
    ...
    def _parse_set_response(self, response, prompt):
        ...
        log.error("SET RESPONSE = " + repr(response))
