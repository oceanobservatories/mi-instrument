...
def validate_response(response, expected_size):
...
class CAMDS_HEALTH_STATUS(DataParticle):
    ...
    def _build_parsed_values(self):
        ...
        #check the response
        if not validate_response(self.raw_data, 11):
            log.error("Health status is not correctly formatted %r" + self.raw_data)
            return
...
class CAMDS_DISK_STATUS(DataParticle):
    ...
    def _build_parsed_values(self):
        ...
        #check the response
        if not validate_response(response_stripped, 15):
