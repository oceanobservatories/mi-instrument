...
class CAMDS_HEALTH_STATUS(DataParticle):
    ...
    def _build_parsed_values(self):
        ...
        parsed_sample = [{DataParticleKey.VALUE_ID: CAMDS_HEALTH_STATUS_KEY.temp,
                          DataParticleKey.VALUE: int(self.raw_data[self.TEMP_INDEX])},
                         {DataParticleKey.VALUE_ID: CAMDS_HEALTH_STATUS_KEY.humidity,
                          DataParticleKey.VALUE: int(self.raw_data[self.HUMIDITY_INDEX])},
                         {DataParticleKey.VALUE_ID: CAMDS_HEALTH_STATUS_KEY.error,
                          DataParticleKey.VALUE: int(self.raw_data[self.ERROR_INDEX])}]
...
class CAMDS_DISK_STATUS(DataParticle):
    ...
    def build_data_particle(self, size, disk_remaining, image_remaining, image_on_disk):

        return [{DataParticleKey.VALUE_ID: CAMDS_DISK_STATUS_KEY.size,
    ...
    def _build_parsed_values(self):
        ...
        byte1 = int(response_stripped[self.BYTE_1_INDEX])
        byte2 = int(response_stripped[self.BYTE_2_INDEX])
        byte3 = int(response_stripped[self.BYTE_3_INDEX])

        available_disk = byte1 * pow(10, byte2)
        available_disk_percent = byte3

        temp = struct.unpack('!h', response_stripped[10:11])
        images_remaining = temp[0]
        temp = struct.unpack('!h', response_stripped[12:13])
        images_on_disk = temp[0]

        parsed_sample = self.build_data_particle(available_disk, available_disk_percent,
                                                 images_remaining, images_on_disk)
