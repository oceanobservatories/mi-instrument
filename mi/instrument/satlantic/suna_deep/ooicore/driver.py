...
class SUNASampleDataParticle(DataParticle):
    ...
    def _build_parsed_values(self, time=None):
        ...
        try:
            ...
            date_str = str(datetime.datetime.date(date))  #%Y-%m-%d

            seconds = str(float(matched.group(4)) * 3600)
            if '.' in seconds:
                seconds, fraction = seconds.split('.', 1)
                fraction = float('.' + fraction)
            else:
                fraction = 0

            t = time.strftime('%H:%M:%S', time.gmtime(float(seconds)))

            timestamp = time.strptime(date_str +t, '%Y-%m-%d%H:%M:%S')
