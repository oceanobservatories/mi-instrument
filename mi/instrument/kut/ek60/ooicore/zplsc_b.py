...
def parse_echogram_file(input_file_path, output_file_path=None):
    ...
    try:
        ...
        log.error('Could not open Raw Echogram file')
        raise
    ...
    if match:
        ...
        log.warn("Unable to extract file time from input file name: %s."
    ...
    while raw:
        ...
        if not trans_array[channel]:
            ...
            if channel == config_header['transducer_count']:
                ...
                year, month, day, hour, minute, second = time.utctimetuple()[:6]
                unix_time = calendar.timegm((year, month, day, hour, minute, second+(time.microsecond/1e6)))
