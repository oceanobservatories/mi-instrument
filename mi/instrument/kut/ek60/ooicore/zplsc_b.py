...
def build_windows_time(high_word, low_word):
    ...
    :return:  time in milliseconds since 1980/01/01 00:00:00 UTC
    """
...
def generate_echogram_plots(td_f, td_dr, trans_array_time, trans_array, output_file_path, first_ping_metadata):
    ...
    for channel in td_f:
        try:
            ...
            os.path.join(output_file_path, first_ping_metadata[ZplscBParticleKey.ECHOGRAM_PATH][channel - 1])

            generate_echogram_plot(
                trans_array_time[channel], trans_array[channel], td_f[channel], td_dr[channel],
                channel, os.path.join(output_file_path,
                                       first_ping_metadata[ZplscBParticleKey.ECHOGRAM_PATH][channel - 1]))
