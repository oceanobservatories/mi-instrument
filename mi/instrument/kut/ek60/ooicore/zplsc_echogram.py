...
class ZPLSPlot(object):
    ...
    def _get_power_range(self, power_dict):
        # Calculate the power data range across all channels
        all_power_data = np.concatenate(power_dict.values())
        max_db = np.nanpercentile(all_power_data, self.upper_percentile)
        min_db = np.nanpercentile(all_power_data, self.lower_percentile)
