...
class ZPLSPlot:
    def __init__(self, trans_array_time, trans_array, td_f, td_dr):
        self.font_size_small = 14
        self.font_size_large = 18
        interplot_spacing = 0.1

        self.trans_array_time = trans_array_time
        ...
        # ranges & increments
        big_data = np.concatenate(self.trans_array.values())
        self.max_db = np.nanpercentile(big_data, 95)
        self.min_db = np.nanpercentile(big_data, 5)
