...
from mi.core.instrument.data_particle import DataParticle, DataParticleValue
...
def get_one_byte_value(str_input, index=0):
    return struct.unpack_from('>B', str_input, index)[0]


def get_two_byte_value(str_input, index=0):
    return struct.unpack_from('>H', str_input, index)[0]


def get_three_byte_value(str_input, index=0):
    return (get_one_byte_value(str_input) << 16) + get_two_byte_value(str_input, index+1)


def get_four_byte_value(str_input, index=0):
    return struct.unpack_from('>I', str_input, index)[0]
...
class OptaaSampleHeader(BigEndianStructure):
    _fields_ = [
        ('packet_registration', c_uint),        # 4
        ('record_length', c_ushort),
        ('packet_type', c_ubyte),
        ('reserved', c_ubyte),                  # 8
        ('meter_type', c_ubyte),
        ('serial_number_high', c_ubyte),
        ('serial_number_low', c_ushort),        # 12
        ('a_ref_dark_counts', c_ushort),
        ('pressure_counts', c_ushort),          # 16
        ('a_signal_dark_counts', c_ushort),
        ('raw_external_temp_counts', c_ushort), # 20
        ('raw_internal_temp_counts', c_ushort),
        ('c_ref_dark_counts', c_ushort),        # 24
        ('c_signal_dark_counts', c_ushort),
        ('time_high', c_ushort),                # 28
        ('time_low', c_ushort),
        ('reserved2', c_ubyte),
        ('num_wavelengths', c_ubyte),           # 32
    ]
...
class OptaaSampleDataParticle(DataParticle):
    ...
    def _build_parsed_values(self):
        ...
        result = [
            ...
            self._encode_value(key.SERIAL_NUMBER, serial_number, identity),
