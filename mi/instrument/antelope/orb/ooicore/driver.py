...
class Protocol(InstrumentProtocol):
    ...
    def _flush(self, last_flush=False):
        log.info('flush')
        particles = []
        with self._lock:
            log.info('got lock')
        ...
        if self.get_current_state() == ProtocolState.STOPPING and not last_flush:
    ...
    def _handler_stopping_exit(self, *args, **kwargs):
        ...
        self._protocol_fsm.on_event(ProtocolEvent.FLUSH, (True))
