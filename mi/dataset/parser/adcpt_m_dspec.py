...
from mi.dataset.parser.common_regexes import UNSIGNED_INT_REGEX, INT_REGEX, FLOAT_REGEX, END_OF_LINE_REGEX, \
    SPACE_REGEX, ONE_OR_MORE_WHITESPACE_REGEX, ANY_CHARS_REGEX
...
# regex for identifying an empty line or screened data
EMPTY_LINE_REGEX = END_OF_LINE_REGEX
EMPTY_LINE_MATCHER = re.compile(EMPTY_LINE_REGEX, re.DOTALL)
SCREENED_DATA_REGEX = r'Screened' + END_OF_LINE_REGEX
SCREENED_DATA_MATCHER = re.compile(SCREENED_DATA_REGEX, re.DOTALL)
...
START_METADATA = ZERO_OR_MORE_WHITESPACE_REGEX + r'\%'
...
DIR_FREQ_PATTERN = START_METADATA + ONE_OR_MORE_WHITESPACE_REGEX + INT_GROUP_REGEX + \
                   ONE_OR_MORE_WHITESPACE_REGEX + r'Directions and' + \
                   ONE_OR_MORE_WHITESPACE_REGEX + INT_GROUP_REGEX + \
                   ONE_OR_MORE_WHITESPACE_REGEX + r'Frequencies,' + \
                   ONE_OR_MORE_WHITESPACE_REGEX + r'Upper CutOff Frequency =' + \
                   ONE_OR_MORE_WHITESPACE_REGEX + FLOAT_GROUP_REGEX + \
                   END_OF_LINE_REGEX
...
FREQ_BAND_PATTERN = START_METADATA + ONE_OR_MORE_WHITESPACE_REGEX + r'Frequency Bands are' + \
                    ONE_OR_MORE_WHITESPACE_REGEX + FLOAT_GROUP_REGEX + ONE_OR_MORE_WHITESPACE_REGEX + \
                    r'Hz wide\(first frequency band is centered at' + ONE_OR_MORE_WHITESPACE_REGEX + \
                    FLOAT_GROUP_REGEX + r'(?:\)*)' + END_OF_LINE_REGEX
...
DSPEC_DATA_REGEX = r'((' + SPACE_REGEX + INT_REGEX + ')+)' + END_OF_LINE_REGEX
...
DSPEC_DATA_MAP = [
    ...
    ('num_dir', 1, int),
    ('num_freq', 2, int),
    ('cut_off_freq', 3, float),
    ('freq_w_band', 4, float),
    ('freq_0', 5, float),
    ('start_dir', 6, float),
    ('directional_surface_spectrum', 7, list)
]
...
class AdcptMDspecParser(SimpleParser):
    ...
    def parse_file(self):
        ...
        if match:
            ...
            error_message = 'Unable to extract file time from DSpec input file name: %s' % input_file_name
        ...
        while line:
            if EMPTY_LINE_MATCHER.match(line):
            ...
            elif SCREENED_DATA_MATCHER.match(line):
                # log a warning message and then cleanly exit the parser
                error_message = 'Screened data found in DSpec input file: %s' % input_file_name
                log.warn(error_message)
                return None
                ...
                for matcher in HEADER_MATCHER_LIST:
                    ...
                    if header_match is not None:
                        ...
                        if matcher is DIR_FREQ_MATCHER:
                            ...
                            cut_off_freq = float(header_match.group(3))
                ...
                values = [np.int64(x) for x in data.split()]  # read in as int64 to avoid overflow
                ...
                if num_values != num_dir:
                    error_message = 'Unexpected Number of directions in line: expected %s, got %s for %s' \
                        % (num_dir, num_values, input_file_name)
                    log.warn(error_message)
                    self._exception_callback(RecoverableSampleException(error_message))
                else:
                ...
                error_message = 'Unexpected data found in line %s for %s' % (line, input_file_name)
                log.warn(error_message)
                self._exception_callback(RecoverableSampleException(error_message))
        ...
        if dspec_matrix_length != num_freq:
            error_message = 'Unexpected Number of frequencies in DSpec Matrix for %s: expected %s, got %s' \
                % (input_file_name, num_freq, dspec_matrix_length)
            log.warn(error_message)
            self._exception_callback(RecoverableSampleException(error_message))
        ...
        parsed_data = [
            ...
            cut_off_freq,  # ('cut_off_freq', 3, float),
            freq_w_band,  # ('freq_w_band', 4, float),
            freq_0,  # ('freq_0', 5, float),
            start_dir,  # ('start_dir', 6, float),
            dspec_matrix  # ('directional_surface_spectrum', 7, list)]
        ]
