...
@file mi-dataset/mi/dataset/parser/test/test_flort_dj.py
@author Rene Gelinas
@brief Test code for flort_dj data parser
...
from mi.dataset.driver.flort_dj.resource import RESOURCE_PATH
...
@attr('UNIT', group='mi')
class FlortDjParserUnitTestCase(ParserUnitTestCase):
    """
    flort_dj Parser unit test suite
    """
    ...
    def create_flort_ctdbp_yml(self):
        path = RESOURCE_PATH
        log.info(path)

        with open(os.path.join(RESOURCE_PATH, 'large_ctdbp_flort_recovered.hex'), 'rU') as file_handle:

            parser = FlortDjParser(file_handle,
                                   self.exception_callback)

            # In a single read, get all particles in this file.
            particles = parser.get_records(150)

            FlortDjParserUnitTestCase.create_yml(particles, 'large_ctdbp_flort_recovered.yml')
    ...
    def test_simple(self):
        ...
        with open(os.path.join(RESOURCE_PATH, 'simple_ctdbp_flort_recovered.hex'), 'rU') as file_handle:
    ...
    def test_large(self):
        ...
        with open(os.path.join(RESOURCE_PATH, 'large_ctdbp_flort_recovered.hex'), 'rU') as file_handle:
            ...
            result = parser.get_records(number_expected_results)
            self.assert_particles(result, 'large_ctdbp_flort_recovered.yml', RESOURCE_PATH)

            self.assertListEqual(self.exception_callback_value, [])

        log.debug('===== END LARGE TEST =====')
