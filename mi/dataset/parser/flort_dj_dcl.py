...
class DataParticleType(BaseEnum):
    ...
    TURBD_A_PARTICLE = 'turbd_a_instrument'
...
class TurbdADclDataParticle(FlortDjDclInstrumentDataParticle):
    """
    Class for generating Data Particles for the TURBD_A.
    """
    _data_particle_type = DataParticleType.TURBD_A_PARTICLE
    ...
    def parse_file(self):
        ...
        for line in self._stream_handle:
            ...
            if sensor_match is not None:
                if self._particle_class._data_particle_type == 'turbd_a_instrument':
                    self._particle_class._data_particle_map = INSTRUMENT_PARTICLE_MAP_TURBD
                    log.debug('TURBD match found')
                else:
