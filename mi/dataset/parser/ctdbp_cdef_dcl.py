...
#
# This regex matches lines from a platform (not just Endurance)  which 
# are generated by a combined CTDBP_CDEF_DCL with a DOSTA_ABCDJM.
# 
...
#
# This regex matches lines from a platform (not just Pioneer)  which 
# are generated by a CTDBP_CDEF_DCL 
# 
...
# with a FLORT_D plugged into it
# 
CTDBP_FLORT_REGEX = TIMESTAMP + ONE_OR_MORE_WHITESPACE_REGEX             # dcl timestamp, named group = timestamp
CTDBP_FLORT_REGEX += '(?:' + HASH + '|' + LOGGER_ID_PATTERN + ')?'        # a logger id or a hash, non-captured group
CTDBP_FLORT_REGEX += ZERO_OR_MORE_WHITESPACE_REGEX
CTDBP_FLORT_REGEX += TEMP_REGEX + COMMA + ONE_OR_MORE_WHITESPACE_REGEX          # named group = temp
CTDBP_FLORT_REGEX += CONDUCTIVITY_REGEX + COMMA + ONE_OR_MORE_WHITESPACE_REGEX  # named group = conductivity
CTDBP_FLORT_REGEX += PRESSURE_REGEX + COMMA + ONE_OR_MORE_WHITESPACE_REGEX      # named group = pressure
CTDBP_FLORT_REGEX += UNSIGNED_INT_REGEX + COMMA + ONE_OR_MORE_WHITESPACE_REGEX  # raw backscatter (omitted)
CTDBP_FLORT_REGEX += UNSIGNED_INT_REGEX + COMMA + ONE_OR_MORE_WHITESPACE_REGEX  # raw chlorophyl (omitted)
CTDBP_FLORT_REGEX += UNSIGNED_INT_REGEX + COMMA + ONE_OR_MORE_WHITESPACE_REGEX  # raw cdom (omitted)
CTDBP_FLORT_REGEX += DATE_TIME_STR + ZERO_OR_MORE_WHITESPACE_REGEX              # named group = time_string
CTDBP_FLORT_REGEX += END_OF_LINE_REGEX
CTDBP_FLORT_MATCHER = re.compile(CTDBP_FLORT_REGEX)
