...
PNG_FILE_EXT = r'.png'
USER_HOME = r'~'
ECHOGRAM_DIRECTORY = r'~/ZPLSC_ECHOGRAMS'
...
class ZPLSCEchogramGenerator(object):
    ...
    def input_is_valid(self, subsites, deployments, echogram_dates):
        ...
        if not zplsc_echrogram_directory:
            self.echogram_directory = ECHOGRAM_DIRECTORY
        else:
            self.echogram_directory = zplsc_echrogram_directory
    ...
    def aggregate_raw_data(self, date_dirs_url, data_date):
        ...
        :return: zplsc_24_datafile: The 34-hour concatenated raw data file
                 zplsc_echogram_file: The file name for the echogram to be generated.
        """
        ...
        if len(filenames_list) == 24:
            ...
            zplsc_echogram_file = zplsc_24_datafile.replace(RAW_FILE_EXT, PNG_FILE_EXT)
        else:
            zplsc_24_datafile = ''
            zplsc_echogram_file = ''

        return zplsc_24_datafile, zplsc_echogram_file
    ...
    def store_echogram(self, zplsc_echogram_file):
        """
        This method will store the generated echogram in the location specified
        in the configuration file. It will then clean up the temporary folder.

        :param zplsc_echogram_file: The generated ZPLSC echogram
        :return:
        """

        # Create the echogram directory, then copy the echogram to the local directory.
        self.echogram_directory = expanduser(self.echogram_directory)
        if not exists(self.echogram_directory):
            mkdir(self.echogram_directory)

        system('cp ' + zplsc_echogram_file + ' ' + self.echogram_directory)
                ...
                for date_dir, entire_month in echogram_dates.items():
                    ...
                    if entire_month:
                        ...
                        for day in range(number_of_days_in_the_month):
                            ...
                            try:
                                zplsc_24_datafile, zplsc_echogram_file = self.aggregate_raw_data(date_dirs_url,
                                                                                                 echogram_date)
                            ...
                            self.store_echogram(zplsc_echogram_file)
                    else:
                        # Aggregate the 24 raw data files for the given instrument to 1 24-hour data file.
                        try:
                            zplsc_24_datafile, zplsc_echogram_file = self.aggregate_raw_data(date_dirs_url, date_dir)
